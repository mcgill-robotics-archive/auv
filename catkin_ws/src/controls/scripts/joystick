...
def joystick():
    ...
    pub = rospy.Publisher('/controls/wrench', geometry_msgs.msg.Wrench, queue_size=1)

    sub = rospy.Subscriber('/cmd_vel', geometry_msgs.msg.Twist, callback_function, queue_size=1)

    rate = rospy.Rate(10)
    while not rospy.is_shutdown():
        rospy.logdebug(wrench_msg)

...
class SuperImposer:
    def __init__(self):
        ...
        rospy.Subscriber(
            "controls/superimposer/surge", Float64,
            lambda data: self.set_surge(data.data))
        rospy.Subscriber(
            "controls/superimposer/sway", Float64,
            lambda data: self.set_sway(data.data))
        rospy.Subscriber(
            "controls/superimposer/heave", Float64,
            lambda data: self.set_heave(data.data))

        # rotation subscribers
        rospy.Subscriber(
            "controls/superimposer/roll", Float64,
            lambda data: self.set_roll(data.data))
        rospy.Subscriber(
            "controls/superimposer/pitch", Float64,
            lambda data: self.set_pitch(data.data))
        rospy.Subscriber(
            "controls/superimposer/yaw", Float64,

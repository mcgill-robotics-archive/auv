...
def move_goal_cb(userdata, goal):
    """Callbacks for move, parameter reading will be moved out of the goal
    callback for an easier time (only the first and last 3 lines of that
    callback will remain)"""
    uid = userdata.move_id

    params = rospy.getparams('~move' + 'id')
    time = RosTime(params.time[0], params.time[1])
    velocity = Twist(Vector3(params.velocity[0],
                             params.velocity[1],
                             params.velocity[2]),
                     Vector3(params.velocity[3],
                             params.velocity[4],
                             params.velocity[5]))
